...
const discColors = {
  understable: { bg: "bg-sky-900/40", border: "border-sky-500", accent: "text-sky-400", badge: "bg-sky-500/20 text-sky-300", dot: "bg-sky-400" },
  stable:      { bg: "bg-emerald-900/40", border: "border-emerald-500", accent: "text-emerald-400", badge: "bg-emerald-500/20 text-emerald-300", dot: "bg-emerald-400" },
  overstable:  { bg: "bg-orange-900/40", border: "border-orange-500", accent: "text-orange-400", badge: "bg-orange-500/20 text-orange-300", dot: "bg-orange-400" },
};
...
// ─── Suggester Logic ─────────────────────────────────────────────────────────
// Each entry: { disc, angle, confidence: "best"|"good"|"risky", summary, tip, aimNote }
			...
			return (
			  <button key={disc} onClick={() => setSelectedDisc(disc)}
				className={`py-2 px-3 rounded border text-sm font-bold uppercase tracking-wide transition-all ${
				  selectedDisc === disc ? `${c.bg} ${c.border} ${c.accent}` : "bg-gray-900 border-gray-700 text-gray-500 hover:border-gray-500"
				}`}>
				{disc}
			  </button>
			);
			...
			return (
			  <button key={angle} onClick={() => setSelectedAngle(angle)}
				className={`py-2 px-3 rounded border text-sm transition-all ${
				  selectedAngle === angle ? "bg-gray-700 border-gray-400 text-white font-bold" : "bg-gray-900 border-gray-700 text-gray-500 hover:border-gray-500"
				}`}>
		  ...
		  {WIND_DIRECTIONS.map((wind) => (
			<button key={wind.id} onClick={() => setSelectedWind(wind.id)}
			  className={`py-3 px-2 rounded border text-center transition-all flex flex-col items-center justify-center gap-1 ${
				selectedWind === wind.id ? "bg-yellow-900/40 border-yellow-500 text-yellow-300" : "bg-gray-900 border-gray-700 text-gray-500 hover:border-gray-500"
			  }`}>
		...
		<div className="grid grid-cols-4 gap-2">
		  {SHOT_SHAPES.map((shot) => (
			<button key={shot.id} onClick={() => setSelectedShot(shot.id)}
			  className={`py-3 px-2 rounded border text-center transition-all flex flex-col items-center justify-center gap-1 ${
				selectedShot === shot.id ? "bg-purple-900/40 border-purple-500 text-purple-300" : "bg-gray-900 border-gray-700 text-gray-500 hover:border-gray-500"
			  }`}>
